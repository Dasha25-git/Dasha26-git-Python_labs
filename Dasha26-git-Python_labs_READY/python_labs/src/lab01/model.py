...
class Book:
    ...
    def __init__(
        ...
        self._price: float = float(price)  # цена м.б. дробной
    ...
    def display(self) -> str:
        """Вернуть строку для структурного протокола Displayable."""
        return str(self)

    def score(self) -> float:
        """Вернуть условную оценку книги для структурного протокола Scorable."""
        availability_bonus = 1.0 if self.available else 0.0
        age_bonus = max(0.0, (2026 - self.year) / 100)
        price_penalty = min(self.price / 5000, 1.0)
        return round(availability_bonus + age_bonus + (1 - price_penalty), 2)
